...
def merge_assembly():
    ...
    path_to_output = sys.argv[1] + "/final"
    paf_file = path_to_output + "/merge_overlaps.paf"
    contigs_file = SeqIO.parse(path_to_output + "/combined.fasta", 'fasta')
    ...
    graph_info_file = path_to_output + "/merge_overlap_graph_log.csv"
    with open(graph_info_file, "w") as csvfile:
        writer = csv.writer(csvfile, delimiter="\t")
        [writer.writerow(r) for r in rows]
    print("status: --- stitching done ---")
    print("----------------- RAmbler stitcher signing off -----------------")
